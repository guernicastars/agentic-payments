...
def compute_features(txs: pd.DataFrame) -> pd.DataFrame:
    ...
    for addr, g in by_wallet.items():
        ...
        feats.update(burst)
        # prompt_injection_share — fraction of wallet's tx flagged as
        # generated by an injected prompt (set upstream by synthetic
        # `prompt_injected_invoice` policy). Default 0 if column absent.
        if "prompt_injection_flag" in g.columns:
            feats["prompt_injection_share"] = float(g["prompt_injection_flag"].mean())
        else:
            feats["prompt_injection_share"] = 0.0
        rows.append(feats)
